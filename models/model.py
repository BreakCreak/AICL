...
class AICL(nn.Module):
    ...
    def select_topk_embeddings(self, scores, embeddings, k, retain_random=0.2):
        ...
        return torch.gather(embeddings, 1, idx_topk)
    ...
    def inconsistency_snippets_mining(self, aness_bin1, aness_bin2, actionness, embeddings, k_hard):
        ...
        gate_center, gate_scale = 0.4, 5.0
        action_gate = torch.sigmoid((actionness - gate_center)*gate_scale)
        hard_act = self.select_topk_embeddings(actionness*idx_region_inner*action_gate, embeddings, k_hard, retain_random=0.2)
        actionness_rev = torch.max(actionness, dim=1, keepdim=True)[0] - actionness
        bg_gate = torch.sigmoid((0.3-actionness)*5)
        hard_bkg = self.select_topk_embeddings(actionness_rev*idx_region_inner+bg_gate*idx_region_inner, embeddings, k_hard, retain_random=0.2)
